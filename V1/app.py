...
# Function to get top-cited papers for a specific dataset
def get_top_cited_papers(query, num_results=10):
    ...
    for i, paper in enumerate(query):
        ...
        results.append((title, citations, url))
    return results
...
def index():
    if request.method == 'POST':
        ...
        filename = f"papers_for_{'_'.join(dataset_names)}_{num_papers}_results.txt"
        ...
        with open(file_path, 'w', encoding='utf-8') as file:
            ...
            for r in range(1, len(dataset_names) + 1):  # r = 1 to len(datasets) for all combinations
                for combination in itertools.combinations(dataset_names, r):
                    ...
                    file.write(f"Results for: {', '.join(combination)}\n")
                    file.write("=" * 50 + "\n")
